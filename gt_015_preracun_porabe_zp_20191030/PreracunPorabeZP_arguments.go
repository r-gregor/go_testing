...
func Usage() {
    fmt.Println(
    "\nUsage: PreracunPorabeZP[.exe] <command argumets>" + "\n")
    
    fmt.Print(
    `   command arguments: OGV nOGV ST nST
            OGV  - Skupna vršna poraba ZP (vsota) za plinske grelnik [m3/h]
            nOGV - Število plinskih grelnikov [-]
            ST   - Skupna vršna poraba ZP (vsota) za plinske štedilnike [m3/h]
            nST  - Število plinskih štedilnikov [-]`)
    fmt.Println("\n\n")
}
...
func main() {
    ...
    if len(os.Args) != 5 {
        ...
        ogv, _ = strconv.ParseFloat(os.Args[1], 64)
        nogv, _ = strconv.Atoi(os.Args[2])
        st, _ = strconv.ParseFloat(os.Args[3], 64)
        nst, _  = strconv.Atoi(os.Args[4])
    }
    ...
    */
    ...
    // poraba := (ogv * float64(nogv) * f_pg.get(nogv) + (st * float64(nst) * f_st.get(nst)))
    poraba := (ogv * f_pg.get(nogv) + st * f_st.get(nst))
    
    // fmt.Printf("Vp = %.3f m3/h * %d * %.3f + %.3f m3/h * %d * %.3f = %.3f m3/h\n", ogv, nogv, f_pg.get(nogv), st, nst, f_st.get(nst), poraba)
    fmt.Printf("Vp = %.3f m3/h * %.3f + %.3f m3/h * %.3f = %.3f m3/h\n", ogv, f_pg.get(nogv), st, f_st.get(nst), poraba)
